...
class Csz_model extends CI_Model
{
    ...
    public function coreCss($more_css = '', $more_include = TRUE)
    {
        ...
        $core_css .= link_tag(base_url('', '', TRUE) . 'assets/css/style.css');
        ...
        $core_css .= link_tag(base_url('', '', TRUE) . 'assets/css/open-iconic-bootstrap.min.css');
        $core_css .= link_tag(base_url('', '', TRUE) . 'assets/css/animate.css');
        $core_css .= link_tag(base_url('', '', TRUE) . 'assets/css/owl.carousel.min.css');
        $core_css .= link_tag(base_url('', '', TRUE) . 'assets/css/owl.theme.default.min.css');
        $core_css .= link_tag(base_url('', '', TRUE) . 'assets/css/magnific-popup.css');
        $core_css .= link_tag(base_url('', '', TRUE) . 'assets/css/aos.css');
        $core_css .= link_tag(base_url('', '', TRUE) . 'assets/css/ionicons.min.css');
        $core_css .= link_tag(base_url('', '', TRUE) . 'assets/css/flaticon.css');
        $core_css .= link_tag(base_url('', '', TRUE) . 'assets/css/icomoon.css');
        $core_css .= link_tag(base_url('', '', TRUE) . 'assets/css/jquery-ui.min.css');
        $core_css .= link_tag(base_url('', '', TRUE) . 'assets/css/jquery.dataTables.min.css');
        $core_css .= link_tag(base_url('', '', TRUE) . 'assets/css/iziToast.min.css');
        $core_css .= link_tag(base_url('', '', TRUE) . 'assets/font-awesome/css/font-awesome.min.css');
    ...
    public function coreJs($more_js = '', $more_include = TRUE)
    {
        ...
        $core_js .= '<script type="text/javascript" src="' . base_url('', '', TRUE) . 'assets/js/aos.js"></script>';
        $core_js .= '<script type="text/javascript" src="' . base_url('', '', TRUE) . 'assets/js/jquery.stellar.min.js"></script>';
        $core_js .= '<script type="text/javascript" src="' . base_url('', '', TRUE) . 'assets/js/jquery.animateNumber.min.js"></script>';         
        $core_js .= '<script type="text/javascript" src="' . base_url('', '', TRUE) . 'assets/js/jquery.easing.1.3.js"></script>';
        $core_js .= '<script type="text/javascript" src="' . base_url('', '', TRUE) . 'assets/js/jquery.waypoints.min.js"></script>';
        $core_js .= '<script type="text/javascript" src="' . base_url('', '', TRUE) . 'assets/js/owl.carousel.min.js"></script>';
        $core_js .= '<script type="text/javascript" src="' . base_url('', '', TRUE) . 'assets/js/scrollax.min.js"></script>';
        $core_js .= '<script type="text/javascript" src="' . base_url('', '', TRUE) . 'assets/js/sweetalert2.all.min.js"></script>';
        $core_js .= '<script type="text/javascript" src="' . base_url('', '', TRUE) . 'assets/js/jquery.dataTables.min.js"></script>';
        $core_js .= '<script type="text/javascript" src="' . base_url('', '', TRUE) . 'assets/js/iziToast.min.js"></script>';
        $core_js .= '<script type="text/javascript" src="' . base_url('', '', TRUE) . 'assets/js/main.js"></script>';
        $core_js .= '<script type="text/javascript" src="' . base_url('', '', TRUE) . 'assets/js/global.js"></script>';

        $core_js .= '<script type="text/javascript">$(function(){$(".lazy").lazy();});$("#sel-chkbox-all").change(function(){$(".selall-chkbox").prop("checked",$(this).prop("checked"))});$(".timepicker").datetimepicker({datepicker:false,step:1,format:"H:i"});</script>';
        ...
        if ($this->getFBsdk() !== FALSE) {
            $core_js .= $this->getFBsdk();
        }
        if (!empty($more_js)) {
            if ($more_include !== FALSE) {
                if (is_array($more_js)) {
        ...
        if ($this->getFBChat() !== FALSE) {
            $core_js .= $this->getFBChat();
        }
        return $core_js;
    }
    ...
    public function coreMetatags($desc_txt, $keywords, $title, $img_path = '', $more_meta = '')
    {
        ...
        if ($twitter !== FALSE && $twitter->social_url) {
            $twitter_username = basename($twitter->social_url);
        }
     ...
     * @return	string
     */
    public function formTagToHTML($content)
     ...
     * @return	string
     */
    public function carouselInHtml($content)
    { /* Find the carousel in content */
        $output_array = array();
        $txt_nonhtml = strip_tags($content);
     ...
     */
    //modificado daniel
    // public function pwidgetInHtml($content)
    // { /* Find the widget in content */
    //     $output_array = array();
    //     $txt_nonhtml = strip_tags($content);
    ...
    public function createMember()
    {
        ...
        if ($config->member_close_regist) {
            ...
            $data = array(
                ...
                'first_name' => $this->input->post('nombres', TRUE),
                'last_name' => $this->input->post('apellidomaterno', TRUE) . ' ' . $this->input->post('apellidopaterno', TRUE),
                'birthday' => $this->input->post('fechanacimiento'),
                'gender' => $this->input->post('sexo', TRUE),
                'address' => $this->input->post('domicilio', TRUE),
                'phone' => $this->input->post('celular', TRUE),
                'user_type' => 'member',
                'active' => 0,
                'md5_hash' => $md5_hash,
                'pm_sendmail' => 1,
            ...
            $data2 = array(
                ...
                'apellido_paterno' => $this->input->post('apellidopaterno', TRUE),
                'apellido_materno' =>  $this->input->post('apellidomaterno', TRUE),
                'carnet' =>  $this->input->post('carnet', TRUE),
                'extension_carnet' =>  $this->input->post('extensioncarnet', TRUE),
                'id_nacionalidad' =>  $this->input->post('nacionalidad', TRUE),
                'lugar_nacimiento' =>  $this->input->post('lugarnacimiento', TRUE),
                'sexo' =>  $this->input->post('sexo', TRUE),
                'domicilio' =>  $this->input->post('domicilio', TRUE),
                'estado_civil' =>  $this->input->post('estadocivil', TRUE),
                'fecha_nacimiento' =>  $this->input->post('fechanacimiento'),
                'celular' =>  $this->input->post('celular', TRUE),
                'telefono' =>  $this->input->post('telefono', TRUE),
                'correo' =>  $this->cleanEmailFormat($this->input->post('email', TRUE)),
                'id_usuario' => $id,
    ...
    public function sendEmail($to_email, $subject, $message, $from_email, $from_name = '', $bcc = '', $reply_to = '', $alt_message = '', $attach_file = array(), $save_log = TRUE)
    {
        ...
        $this->email->from('info@contratoslegal.com', $from_name); // change it to yours
        $this->email->to('bruno.poehlmann@gmail.com'); // change it to yours
    ...
    {
        $row = $this->Csz_model->getValueArray('*', 'person', "id_usuario = '" . $id . "'", 0);
        if ($row !== FALSE) {
            return $row;
        } else {
            return FALSE;
    ...
    public function obtenerContratos($id)
    {
        $sql = 'SELECT c.id,c.fecha_creacion,c.id_usuario_creacion,c.estado,c.id_usuario, c.id_cliente,c.id_garante,'.
        ' CONCAT_WS(" ",p1.nombres,p1.apellido_paterno,p1.apellido_materno) as vendendor,'.
        ' CONCAT_WS(" ",p2.nombres,p2.apellido_paterno,p2.apellido_materno) as cliente,'.
        ' con.Nombre as Contrato'.
        ' FROM maestro_contrato c'.
        ' right join person p1 on c.id_cliente = p1.id'.
        ' right join person p2 on c.id_usuario = p2.id'.
        ' inner join contrato con on c.id_contrato = con.IdContrato'.
        ' WHERE id_usuario_creacion ='. $id;

        $query= $this->db->query($sql);
        return $query->result_array();
    }
    public function guardarPersona($get)
    {
        if ((int)($get['id']) != 0) {
            $this->db->set('nombres', ($get['nombres']));
            $this->db->set('apellido_paterno', ($get['apellidopaterno']));
            $this->db->set('apellido_materno', ($get['apellidomaterno']));
            $this->db->set('carnet', ($get['carnet']));
            $this->db->set('extension_carnet', ($get['expedido']));
            $this->db->set('id_nacionalidad', ($get['nacionalidad']));
            $this->db->set('lugar_nacimiento', ($get['lugarnacimiento']));
            $this->db->set('sexo', ($get['sexo']));
            $this->db->set('estado_civil', ($get['estadocivil']));
            $this->db->set('domicilio', ($get['domicilio']));
            $this->db->set('fecha_nacimiento', ($get['fechanacimiento']));
            $this->db->set('celular', ($get['celular']));
            $this->db->set('telefono', ($get['telefono']));
            $this->db->set('correo', ($get['correo']));
            ...
            $data = array(
                'nombres' => ($get['nombres']),
                'apellido_paterno' => ($get['apellidopaterno']),
                'apellido_materno' => ($get['apellidomaterno']),
                'carnet' => ($get['carnet']),
                'extension_carnet' => ($get['expedido']),
                'id_nacionalidad' => ($get['nacionalidad']),
                'lugar_nacimiento' => ($get['lugarnacimiento']),
                'sexo' => ($get['sexo']),
                'estado_civil' => ($get['estadocivil']),
